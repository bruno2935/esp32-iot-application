...
const int WIFI_APP_USER_REQUESTED_STA_DISCONECT_BIT		    = BIT2;
const int WIFI_APP_STA_CONNECT_GOT_IP_BIT					= BIT3;


// Queue handle used to manipule the main queue of event
...
/**
 * Initialize the wifi application event handler for Wifi and IP event_groups
 */
...
static void wifi_app_soft_ap_config(void)
{
	...
	// This 2 nexts lines convert the ip format to binary format
	inet_pton(AF_INET,	WIFI_AP_IP,	&ap_ip_info.ip);   ///> Assign access point's, Gateway, and netmask
	inet_pton(AF_INET,	WIFI_AP_GATEWAY,	&ap_ip_info.gw);   ///> Assign access point's, Gateway, and netmask
	inet_pton(AF_INET,	WIFI_AP_NETMASK,	&ap_ip_info.netmask);   ///> Assign access point's, Gateway, and netmask
		...
		if(xQueueReceive(wifi_app_queue_handle,	&msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
					...
					// Let the HTTP server knows about  the conncection attempt
				...
				case WIFI_APP_MSG_STA_CONNECT_GOT_IP:
					ESP_LOGI(TAG, "WIFI_APP_MSG_STA_CONNECT_GOT_IP");
					
					xEventGroupSetBits(wifi_app_event_group, WIFI_APP_STA_CONNECT_GOT_IP_BIT);

					rgb_led_wifi_connected();
					http_server_monitor_send_message(HTTP_MSG_WIFI_CONNECT_SUCCESS);

					eventBits = xEventGroupGetBits(wifi_app_event_group);
					if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT) /// Save sta creds only if connecting from the HTTP server	
					{
				...
				case WIFI_APP_MSG_USER_REQUESTED_STA_DISCONECT:
					ESP_LOGI(TAG, "WIFI_APP_MSG_USER_REQUESTED_STA_DISCONECT");
					
					eventBits = xEventGroupGetBits(wifi_app_event_group);
					
					if (eventBits & WIFI_APP_STA_CONNECT_GOT_IP_BIT)
					{
						
						xEventGroupSetBits( wifi_app_event_group, WIFI_APP_USER_REQUESTED_STA_DISCONECT_BIT);
					
						g_retry_number = MAX_CONNECTION_RETRIES; 
						ESP_ERROR_CHECK(esp_wifi_disconnect());
						app_nvs_clear_sta_creds();
						rgb_led_http_server_started();  ///> rename this status LED to a name more meaningfull
					}
				...
				case WIFI_APP_MSG_STA_DISCONNECTED:
					...
					if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT)
					{
						...
						xEventGroupClearBits(wifi_app_event_group, WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER_BIT);
						http_server_monitor_send_message(HTTP_MSG_WIFI_CONNECT_FAIL);
					}else if (eventBits & WIFI_APP_USER_REQUESTED_STA_DISCONECT_BIT)
					{
						ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECTED: USER REQUESTED DISCONNECTION");
						xEventGroupClearBits(wifi_app_event_group, WIFI_APP_MSG_USER_REQUESTED_STA_DISCONECT);
						...
						// Ajust this case to your needs - maybe you want to keep trying to connect...
					}

					if(eventBits & WIFI_APP_STA_CONNECT_GOT_IP_BIT)
					{
						xEventGroupClearBits(wifi_app_event_group, WIFI_APP_MSG_STA_CONNECT_GOT_IP);
					}
...
void wifi_app_start(void)
{
	...
	xTaskCreatePinnedToCore(&wifi_app_task,	"Wifi_app_taks", WIFI_APP_TASK_STACK_SIZE, NULL, WIFI_APP_TASK_PRIORITY, NULL, WIFI_APP_TASK_CORE_ID);
