...
/**
 * ISR handle for the Wifi reset (BOOT) button
 * @param arg parameter which can be passed to the ISR Handle.
 */
 ...
 * to the wifi application to disconnect from wifi clear the saved credentials.
...
void wifi_reset_button_config(void)
{
	...
	// Configure button and set the direction
	...
	xTaskCreatePinnedToCore(&wifi_reset_button_task , "Wifi_reset_button", WIFI_RESET_BUTTON_TASK_STACK_SIZE, NULL, WIFI_RESET_BUTTON_TASK_PRIORITY, NULL, WIFI_RESET_BUTTON_CORE_ID);
